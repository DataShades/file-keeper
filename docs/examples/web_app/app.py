...
if STORAGE_TYPE == "memory":
    ...
    storage = fk.make_storage("web_app", {"type": "file_keeper:memory"})
elif STORAGE_TYPE == "fs":
    # Use filesystem storage for production
    storage = fk.make_storage(
        "web_app", {"type": "file_keeper:fs", "path": os.getenv("STORAGE_PATH", "./uploads"), "initialize": True}
    )
else:
...
def get_file(filename):
    ...
    try:
        ...
        return send_file(buffer, mimetype=content_type, as_attachment=True, download_name=file_info.location)
...
def health_check():
    ...
    try:
        ...
        return jsonify(
            {"status": "healthy", "storage_type": STORAGE_TYPE, "capabilities": str(storage.capabilities)}
        ), 200
    except Exception as e:
