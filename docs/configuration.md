...
| `overwrite_existing`     | bool      | `False`     | If `True`, existing files will be overwritten during upload. If `False`, an `ExistingFileError` will be raised if a file with the same location already exists.                                                                                                          |
